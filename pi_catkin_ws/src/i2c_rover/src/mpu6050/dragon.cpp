...
Dragon::Dragon(int port, pthread_rwlock_t *rwlock, glove_state *p_state)
    : port(port), rwlock(rwlock), p_state(p_state) {}
  ...
  while (!should_landing) {
    ...
    if (n <= 0) {
      ...
      case PT_GRAB_GLOVE_STATE:
        ...
        sendto(sock, sendbuf, total_len, 0, (struct sockaddr *)&peeraddr,
               peerlen);
        break;
      default:
        break;
...
void Dragon::landing() { this->should_landing = true; }