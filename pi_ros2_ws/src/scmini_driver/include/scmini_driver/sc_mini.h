...
public:
  ...
  /**
   * @brief Poll the laser to get a new scan. Blocks until a complete new scan
   * is received or close is called.
   * @param scan LaserScan message pointer to fill in with the scan. The caller
   * is responsible for filling in the ROS timestamp and frame_id
   */
  int poll(sensor_msgs::LaserScan &Scan, int fd);
  void PointCloudFilter(sensor_msgs::LaserScan &Scan);
  void angle_insert(sensor_msgs::LaserScan &scan_in,
...
private:
  ...
  bool shutting_down_; ///< @brief Flag for whether the driver is supposed to be
                       ///< shutting down or not
  uint16_t
      motor_speed_; ///< @brief current motor speed as reported by the LFCD.
};
